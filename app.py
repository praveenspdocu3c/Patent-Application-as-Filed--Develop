...
def generate_analysis_prompt(extracted_details, foundational_claim, dependent_claim, figure_analysis):  
    ...
    - Important: Ensure that all proposed amendments are fully supported by the original disclosure from application as filed only.  
    ...
    - Important: **Underline** the new or modified language by enclosing it within `<u>` and `</u>` tags.  
      (Important Note the Source Reference of the new or modified language by enclosing it within `<u>` and `</u>` tags for later use)
    - The amendments should not be restricted to introducing new features, specific materials, or configurations. Consider all types of improvements that enhance the claim while staying within the original disclosure from application as filed (Note the Source Reference of the new or modified language by enclosing it within `<u>` and `</u>` tags for later use) only.  

    5. Source Reference:
        - Ensure that all proposed amendments `<u>` Mentioned Content `</u>` tags are fully supported by the application as filed only.
        - Source Reference: Carefully cite the exact sections, paragraphs, figures, or embodiments from the Application as Filed that directly support the Proposed Amended `<u>` Mentioned Content `</u>` tags. Ensure the citations accurately correspond to the content used in the amendment. 
        ...
        Note: Before finalizing, cross-check all source references to ensure they match the provided Application as Filed content to support Proposed Amended `<u>` Mentioned Content `</u>` tags.
        Note: Always provide the Paragraph number to the Source Reference.
    ...
    Source Reference: Carefully cite the exact sections, paragraphs, figures, or embodiments from the Application as Filed that directly support the Proposed Amended `<u>` Mentioned Content `</u>` tags. Ensure the citations accurately correspond to the content used in the amendment. 
    Source Reference: For example, "Derived from Paragraph [0032] of the application, which describes the specific configuration..." or "Supported by Figure 4, illustrating the component layout..."
    Important Note Verification Step: Before finalizing, cross-check all source references to ensure they match the provided Application as Filed content to support Proposed Amended `<u>` Mentioned Content `</u>` tags.
    Reasoning: 
